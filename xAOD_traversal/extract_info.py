...
def traverse_daods(file_name):
    ...
    def parse_file_name(file_name):
      output_file_name = ""
      jzxw_search = re.compile('(JZ\dW)') 
      zprimexxx_search = re.compile('(zprime\d+)') 
      daod_num_search = re.compile('(\d+).pool.root')

      jzxw = jzxw_search.search(file_name)
      zprimexxx = zprimexxx_search.search(file_name)
      daod_num = daod_num_search.search(file_name)
        ...
        if(t.AntiKt10LCTopoTrimmedPtFrac5SmallR20Jets.at(i).pt()>=150e3 and abs(t.AntiKt10LCTopoTrimmedPtFrac5SmallR20Jets.at(i).eta())< 2.7):
          ...
          if(topos.isValid()):
              ...
              #for index in xrange(topos.size()):
                    ...
                    #rint( "Constit:     pt = %g, eta = %g, phi = %g" % (topo.pt(), topo.eta(), topo.phi()))
                    #l = dir(topo)
                    #pprint(l)
